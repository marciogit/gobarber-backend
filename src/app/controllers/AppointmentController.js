...
class AppointmentController {
	...
	async store(req, res) {
		...
		if (!isProvider) {
			return res.status(401).json({ error: 'You can only make an appoint with a provider' });
		}
		...
		if (checkAvailability) {
			return res.status(401).json({ error: 'The date is not available' });
		}
	...
	async delete(req, res) {
		...
		// Fazendo a subtração de 2h da hora do appointment
		const dateSubHours = subHours(appointment.date, 2);

		if (isBefore(dateSubHours, new Date())) {
			return res.status(401).json({ error: 'You can only cancel your appointment 2h before your appointment' });
		}
